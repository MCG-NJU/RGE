...
from qwen_vl_utils import fetch_video, fetch_image
...
def process_to_train_qwen2_5_vl_with_default_chat_template(
    ...
    for i, message in enumerate(messages):
        ...
        if isinstance(message['content'], str):
            ...
            # BEGIN: make self generated <emb> token a new EOS token
            if emb_token_type is not None:
                if message['role'] == 'user':
                    if emb_token_type == 'question':
                        output.append(["<|im_end|>\n", False])
                        # output.append(["<emb><rationale><|im_end|>\n", False])
                    elif emb_token_type == 'emb-only':
                        output.append(["<|im_end|>\n", False])
                    else:
                        raise ValueError(f"Invalid emb_token_type: {emb_token_type}")
            # output.append(["<im_end>", message['role'] == 'assistant']) # NOTE: remove this eos token
            if message['role'] == 'assistant':
                output.append(["<emb>", True])
            # output.append(["\n", False]) # NOTE: remove this \n token
            # END: make self generated <emb> token a new EOS token
    ...
    # tokenize texts inputs and get labels for training
