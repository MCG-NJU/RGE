...
def apply_simple_text_ops(text, rules):
    """Apply simple composable text ops: replace, prefix_add, suffix_add.

    Expected keys in `rules` (all optional):
      - original: str      # replace target
      - replacement: str   # replace with
      - prefix_add: str    # add to start
      - suffix_add: str    # add to end
      - value_map: dict    # exact match mapping, e.g., {"Yes": "Yes, it contains ..."}
    Ops run in order: replace -> prefix_add -> suffix_add.
    """
    ...
    # Optional exact-value mapping first (keeps logic simple)
    value_map = rules.get("value_map")
    if isinstance(value_map, dict) and text in value_map:
        text = value_map[text]

    original = rules.get("original")
    replacement = rules.get("replacement", "")
    if original is not None:
        text = text.replace(original, replacement)

    prefix_val = rules.get("prefix_add")
    if isinstance(prefix_val, str) and prefix_val:
        text = f"{prefix_val}{text}"

    suffix_val = rules.get("suffix_add")
    if isinstance(suffix_val, str) and suffix_val:
...
class EvalDataset(Dataset):
    ...
    def __getitem__(self, item):
        ...
        if self.model_args.model_backbone != PHI3V:
            text = text.replace(vlm_image_tokens[PHI3V], "<|vision_start|><|image_pad|><|vision_end|>")
        if "<|vision_start|><|image_pad|><|vision_end|>" in text and img_path == "":
...
class ConfigDataset(TaskBatchDataset):
    def __init__(self, data_args, model_args, training_args):
        ...
        with open(self.data_args.instruction_replacements, 'r') as yaml_file:
            self.instruction_replacements = yaml.safe_load(yaml_file)
        with open(self.data_args.candidate_replacements, 'r') as yaml_file:
            self.candidate_replacements = yaml.safe_load(yaml_file)
        with open(self.data_args.dataset_config, 'r') as yaml_file:
            dataset_config = yaml.safe_load(yaml_file)
        for data_idx, (global_dataset_name, dataset_config_item) in enumerate(dataset_config.items()):
            num_sample = dataset_config_item.get('num_sample_per_subset', -1)
            dataset_name = dataset_config_item.get('dataset_name', '')
            subset_name = dataset_config_item.get('subset_name', '')
            if "rationale" in dataset_name:
                # NOTE: temporary usage of generated json files
                subset_data = load_dataset("json", data_files=f"{dataset_name}/{subset_name}.json", split=f"train[:{num_sample}]")
            else:
                subset_data = load_dataset(
                    dataset_name,
                    subset_name,
            ...
            with self.training_args.main_process_first(local=False, desc="Processing dataset"):
                # Change to proper instructions
                subset_data = subset_data.map(lambda x: {
                    "qry": ensure_trailing_newline(
                        apply_simple_text_ops(x["qry"], self.instruction_replacements.get(subset_name, {}))
                    ),
                    "pos_text": ensure_trailing_newline(
                        apply_simple_text_ops(x["pos_text"], self.candidate_replacements.get(subset_name, {}))
                    ),
                }, num_proc=8)
            self.subset_datasets.append(subset_data)
    ...
    def __getitem__(self, idx):
        ...
        rationale = item_data["rationale"] if "rationale" in item_data else None

        if self.model_args.model_backbone != PHI3V:
            qry_text = qry_text.replace(vlm_image_tokens[PHI3V], vlm_image_tokens[self.model_args.model_backbone])
            pos_text = pos_text.replace(vlm_image_tokens[PHI3V], vlm_image_tokens[self.model_args.model_backbone])
        
        all_texts = [qry_text, pos_text]
        all_image_paths = [qry_image_path, pos_image_path]
        
        for idx, (text, img_path) in enumerate(zip(all_texts, all_image_paths)):
            if vlm_image_tokens[self.model_args.model_backbone] in text and img_path == "":
                all_texts[idx] = text.replace(vlm_image_tokens[self.model_args.model_backbone], "")
        
        qry_text, pos_text = all_texts
        return (qry_text, self._get_image(qry_image_path),
                pos_text, self._get_image(pos_image_path),
                [], [], 
                rationale,
...
class NewPromptsEvalDataset(EvalDataset):
    def __init__(self, data_args, model_args, subset, text_field, img_path_field):
        logger.info(f"Initializing EvalDataset for {subset}")
        logger.info(f"Data args: {data_args}")
        logger.info(f"Model args: {model_args}")
        ...
        def _process_text_field(field_value, replacement_rules):
            if isinstance(field_value, List):
                return [
                    ensure_trailing_newline(
                        apply_simple_text_ops(item, replacement_rules)
                    )
                    for item in field_value
                ]
            return ensure_trailing_newline(
                apply_simple_text_ops(field_value, replacement_rules)
            )

        self.eval_data = self.eval_data.map(lambda x: {
            "qry_text": _process_text_field(
                x["qry_text"], self.instruction_replacements.get(subset, {})
            ),
            "tgt_text": _process_text_field(
                x["tgt_text"], self.candidate_replacements.get(subset, {})
            ),
        }, num_proc=10)
