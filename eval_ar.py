...
from src.dataset import EvalDataset, NewPromptsEvalDataset
from src.collator import EvalCollator, AREvalCollator
...
import yaml
from src.dataset import ensure_trailing_newline, apply_simple_text_ops
...
def main():
    ...
    processor.tokenizer.padding_side = 'left' #"right"
    model = MMEBModel.load(model_args, processor=processor)
    model.eval()
    model = model.to(training_args.device, dtype=torch.bfloat16)

    qry_collator = AREvalCollator(
        data_args=data_args,
    ...
    for idx, subset in enumerate(data_args.subset_name):
        ...
        dataset_func = NewPromptsEvalDataset #EvalDataset
        ...
        if not os.path.exists(encode_qry_path):
                ...
                for batch_idx, batch in enumerate(tqdm(eval_qry_loader)):
                    ...
                    with torch.autocast(enabled=True, dtype=torch.bfloat16, device_type="cuda"):
                        ...
                        for text, image in zip(batch['text'], batch['image']):
                            qry_inputs = processor(text=text, images=image, return_tensors="pt").to(training_args.device)
                            rep_t, generated_text = model.encode_and_generate(qry_inputs)
                            batch_reps.append(rep_t)
                            generated_text_list.append(generated_text)
                        batch_reps = torch.cat(batch_reps, 0)
                        encoded_tensor.append(batch_reps.cpu().detach().float().numpy())
            ...
            # save to jsonl
            with open(os.path.join(output_path, f"{subset}_generated.jsonl"), "w") as f:
                for text in generated_text_list:
                    f.write(json.dumps({"generated_text": text}) + "\n")
    ...
    for subset in tqdm(data_args.subset_name, desc="calculate score"):
        ...
        with open(data_args.instruction_replacements, 'r') as yaml_file:
            instruction_replacements = yaml.safe_load(yaml_file)
        with open(data_args.candidate_replacements, 'r') as yaml_file:
            candidate_replacements = yaml.safe_load(yaml_file)
        ...
        eval_data = eval_data.map(lambda x: {
            "qry_text": _process_text_field(
                x["qry_text"], instruction_replacements.get(subset, {})
            ),
            "tgt_text": _process_text_field(
                x["tgt_text"], candidate_replacements.get(subset, {})
            ),
        }, num_proc=10)
